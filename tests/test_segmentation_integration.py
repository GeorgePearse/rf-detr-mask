            ...
            if hasattr(detections, "mask") and detections.mask is not None:
                ...
                if len(mask_shape) == 3:
                    ...
                    if mask_height == height and mask_width == width:
                        print("✓ Mask dimensions match image size")
                    else:
                        print(
                            f"✗ Mask dimensions ({mask_height}x{mask_width}) don't match image size ({height}x{width})"
                        )
                        return False
                else:
...
def test_batch_processing():
    ...
    try:
        ...
        print(
            f"✓ Batch detection without masks successful. Processed {len(detections_list)} images"
        )
        ...
        print(
            f"✓ Batch detection with masks successful. Processed {len(detections_list_with_masks)} images"
        )
...
def test_visualization():
    ...
    try:
        ...
        detection_result = model.predict(
            pil_image, threshold=0.3, return_masks=True
