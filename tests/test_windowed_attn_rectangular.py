...
class TestWindowedAttentionRectangular(unittest.TestCase):
    ...
    def test_windowed_view_operation(self):
        ...
        self.assertEqual(
            original_size, windowed_size, "Total tensor size must be preserved for view operation"
        )
        ...
        for h in range(num_h_patches_per_window):
            for w in range(num_w_patches_per_window):
                expected_value = (h + num_h_patches_per_window) * 100 + (
                    w + num_w_patches_per_window
                )
                self.assertEqual(
                    window_1_1[h, w].item(),
                    expected_value,
        ...
        try:
            ...
            for h in range(num_h_patches_per_window):
                for w in range(num_h_patches_per_window):  # Note: w is limited by h dimension
                    ...
                    if (
                        w < num_h_patches_per_window
                        and window_0_0_buggy[h, w].item() != expected_correct_value
                    ):
            ...
            if num_h_patches_per_window != num_w_patches_per_window:
                self.assertTrue(
                    discrepancy_found,
                    "Buggy version should rearrange data incorrectly for rectangular inputs",
                )
