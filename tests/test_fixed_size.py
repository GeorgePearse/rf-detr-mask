...
class TestFixedSize(unittest.TestCase):
    ...
    def test_fixed_size_dataset(self):
        ...
        # Create dataset
        dataset = FixedSizeDataset(num_samples=5, resolution=self.args.resolution, num_classes=self.args.num_classes)
        self.assertEqual(len(dataset), 5, "Dataset should have 5 samples")
        
        # Test getting a sample
        image, mask, target = dataset[0]
        self.assertEqual(image.shape, (3, self.args.resolution, self.args.resolution), "Image should have correct shape")
        self.assertEqual(mask.shape, (self.args.resolution, self.args.resolution), "Mask should have correct shape")
