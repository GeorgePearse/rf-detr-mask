...
class TestCheckpointLoading(unittest.TestCase):
    ...
    def test_detection_head_reinitialization(self):
        ...
        if model.two_stage:
            num_groups = model.group_detr
            for i in range(num_groups):
                self.assertEqual(
                    model.transformer.enc_out_class_embed[i].bias.shape[0], 
                    checkpoint_num_classes,
                    f"Group {i} class embedding size mismatch"
                )
    ...
    def test_mask_embedding_initialization(self):
        ...
        self.assertTrue(torch.all(model.mask_embed.layers[-1].weight.data == 0), 
                        "Final mask embed layer weights should be initialized to zero")
        self.assertTrue(torch.all(model.mask_embed.layers[-1].bias.data == 0), 
        ...
        self.assertEqual(mask_pred.shape, (1, batch_size, num_queries, 28, 28), 
