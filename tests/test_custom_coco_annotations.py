    ...
    def test_custom_annotations(self):
        ...
        try:
            ...
            if len(train_dataset) > 0:
                ...
                self.assertIn("labels", sample[1])
        except Exception as e:
            print(f"✗ Failed to load train dataset: {e}")
            self.fail(f"Failed to load train dataset: {e}")
        ...
        try:
            ...
            self.fail(f"Failed to load val dataset: {e}")
        ...
        try:
            ...
            self.assertGreater(len(train_dataset_abs), 0)
        except Exception as e:
            print(f"✗ Failed to load train dataset with absolute path: {e}")
            self.fail(f"Failed to load train dataset with absolute path: {e}")
        ...
        try:
            ...
            # We don't assert here as this might fail and that's expected
        except Exception as e:
            print(f"✗ Failed to load default train dataset (expected): {e}")
            # This is an expected failure, so no assertion
