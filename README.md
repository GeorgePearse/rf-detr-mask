...
- `2025/05/11`: We release RF-DETR-MASK, extending RF-DETR with instance segmentation capabilities through an added mask head.
...
RF-DETR-MASK is currently only available from source. Install it in a [**Python>=3.9**](https://www.python.org/) environment:

```bash
pip install git+https://github.com/roboflow/rf-detr-mask.git
```
...
```bash
git clone https://github.com/roboflow/rf-detr-mask.git
cd rf-detr-mask
pip install -e ".[dev]"
```
...
# Using both box and mask annotators
...
RF-DETR-MASK is available in two variants, just like the original RF-DETR:
- RF-DETR-MASK-B 29.1M [`RFDETRBase`](https://github.com/roboflow/rf-detr-mask/rfdetr/detr.py) (base model with mask head)
- RF-DETR-MASK-L 128.1M [`RFDETRLarge`](https://github.com/roboflow/rf-detr-mask/rfdetr/detr.py) (large model with mask head)

Note that the mask head adds only a modest number of parameters to the original models. The corresponding COCO pretrained checkpoints for instance segmentation will be automatically loaded when you initialize either class.
...
RF-DETR supports exporting models to PyTorch format. To export your model, simply initialize it and call the `.export()` method.
...
This command saves the PyTorch model to the `output` directory.
