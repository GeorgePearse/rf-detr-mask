...
def build_dataset(image_set, args, training_width=None, training_height=None):
    ...
    if hasattr(args, "training_width") and hasattr(args, "training_height"):
        ...
        val_limit_test_mode = getattr(args, "val_limit_test_mode", 20)
        
        # If in test mode, use the test mode validation limit
        if test_mode and image_set == "val":
            limit = val_limit_test_mode
        else:
