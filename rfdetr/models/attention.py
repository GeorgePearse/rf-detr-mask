...
def _generate_qkv(q, k, v, num_heads, key_padding_mask=None):
    ...
    cu_seqlens_q = torch.arange(
        0, (batch_size + 1) * tgt_len, step=tgt_len, dtype=torch.int32, device=q_unpad.device
    )
    max_seqlen_q = tgt_len

    def output_pad_fn(output_unpad):
        return rearrange(output_unpad, "(b t) h d -> b t h d", b=batch_size)
    ...
    if key_padding_mask is not None:
        ...
        cu_seqlens_k = torch.arange(
            0, (batch_size + 1) * src_len, step=src_len, dtype=torch.int32, device=q_unpad.device
        )
