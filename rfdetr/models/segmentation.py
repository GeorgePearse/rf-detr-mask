...
class MaskHeadSmallConv(nn.Module):
    ...
    def __init__(
        self, dim: int, context_dim: int, hidden_dim: int = 256, num_groups: int = 8
    ):
        super().__init__()
...
class RFDETRSegmentation(nn.Module):
    ...
    def __init__(
        self, detr: nn.Module, freeze_detr: bool = False, hidden_dim: int = 256
    ):
        """
        Args:
    ...
    def forward(self, samples) -> Dict[str, Tensor]:
        ...
        memory = self.detr.transformer.encoder(src, src_key_padding_mask=mask)
        ...
        attention_out, attention_weights = self.bbox_attention(
            query_embed, src_flat, src_flat
        )
        ...
        outputs = {
            ...
            "pred_masks": mask_logits.sigmoid(),  # [B, N, H/4, W/4]
        }
