...
class OnnxOptimizer:
    ...
    def find_node_input(self, node, name: Optional[str] = None, value=None) -> int:
    ...
    def find_node_output(self, node, name: Optional[str] = None, value=None) -> int:
    ...
    def fuse_kv(self, node_k, node_v, fused_kv_idx, heads, num_dynamic=0):
        ...
        constant_weights_kv = gs.Constant(
            f"Weights_KV_{fused_kv_idx}", np.ascontiguousarray(weights_kv)
        )
    ...
    def insert_fmhca(self, node_q, node_kv, final_tranpose, mhca_idx, heads, num_dynamic=0):
        ...
        if num_dynamic > 0:
            reshape2_input1_out = gs.Variable(
                f"Reshape2_fmhca{mhca_idx}_out", np.dtype(np.int64), None
            )
    ...
    def fuse_qkv(self, node_q, node_k, node_v, fused_qkv_idx, heads, num_dynamic=0):
        ...
        constant_weights_qkv = gs.Constant(
            f"Weights_QKV_{fused_qkv_idx}", np.ascontiguousarray(weights_qkv)
        )
