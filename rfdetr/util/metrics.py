...
class MetricsPlotSink:
    ...
    def save(self):
        ...
        ap50_90 = np.array(
            [safe_index(x, 0) for x in test_coco_eval if x is not None], dtype=np.float32
        )
        ap50 = np.array(
            [safe_index(x, 1) for x in test_coco_eval if x is not None], dtype=np.float32
        )
        ar50_90 = np.array(
            [safe_index(x, 8) for x in test_coco_eval if x is not None], dtype=np.float32
        )

        ema_coco_eval = [
            h["ema_test_coco_eval_bbox"] for h in self.history if "ema_test_coco_eval_bbox" in h
        ]
        ...
        if len(epochs) > 0:
            ...
            if len(test_loss):
                axes[0][0].plot(
                    epochs, test_loss, label="Validation Loss", marker="o", linestyle="--"
                )
        ...
        if ap50.size > 0 or ema_ap50.size > 0:
            if ap50.size > 0:
                axes[0][1].plot(
                    epochs[: len(ap50)], ap50, marker="o", linestyle="-", label="Base Model"
                )
            if ema_ap50.size > 0:
                axes[0][1].plot(
                    epochs[: len(ema_ap50)], ema_ap50, marker="o", linestyle="--", label="EMA Model"
                )
        ...
        if ap50_90.size > 0 or ema_ap50_90.size > 0:
            if ap50_90.size > 0:
                axes[1][0].plot(
                    epochs[: len(ap50_90)], ap50_90, marker="o", linestyle="-", label="Base Model"
                )
        ...
        if ar50_90.size > 0 or ema_ar50_90.size > 0:
            if ar50_90.size > 0:
                axes[1][1].plot(
                    epochs[: len(ar50_90)], ar50_90, marker="o", linestyle="-", label="Base Model"
                )
